...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 19 15:15:20 2020
@ Copy From : https://www.jianshu.com/p/9af2ae74ec04, All Copy Right and Contribution are reserved from  'Larry xiaochus'.
@@ MobileNet V3 implementation in keras.
"""
...
class MobileNetBase:
    ...
    def _relu6(self, x):
        """Relu 6
        """
        return KTF.relu(x, max_value=6.0)

    def _hard_swish(self, x):
        """Hard swish
        """
        return x * KTF.relu(x + 3.0, max_value=6.0) / 6.0
    ...
    def _conv_block(self, inputs, filters, kernel, strides, nl):
        ...
        channel_axis = 1 if KTF.image_data_format() == 'channels_first' else -1
    ...
    def _bottleneck(self, inputs, filters, kernel, e, s, squeeze, nl):
        ...
        channel_axis = 1 if KTF.image_data_format() == 'channels_first' else -1
        input_shape = KTF.int_shape(inputs)
